...
public class ActiveLoanDetailsActivity extends AppCompatActivity {
        ...
        return new Callback<List<ActiveLoanRecord>>() {
            ...
            public void runResultOnUiThread(List<ActiveLoanRecord> result) {
                ...
                if (activeLoanRecordList.isEmpty()) {
                    int monthlyRate = (int)getMonthlyInterestEffectively(loanRequest.getTotalAmount(), loanRequest.getPeriod(), loanRequest.getInterestRate()/10);
    ...
    public static double getMonthlyInterestEffectively(double totalCreditAmount, double nrMonths, double interestRatePercent) {
        double sum = 0;
